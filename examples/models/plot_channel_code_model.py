...
from kaira.modulations import BPSKModulator, BPSKDemodulator
from kaira.models.binary import RepetitionEncoder, MajorityVoteDecoder
...
with torch.no_grad():
    ...
    for i, snr in enumerate([0, 3, 6, 10]):
        ...
        plt.stem(np.arange(len(modulated_plot)), modulated_plot.numpy(), 
                markerfmt='bo', linefmt='b-', basefmt='b-', label='Transmitted')
        
        # Plot received signal
        plt.plot(np.arange(len(received_plot)), received_plot.numpy(), 
                'r.', alpha=0.7, label='Received')
