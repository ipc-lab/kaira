...
sequential_model = SequentialModel(
    steps=[encoder, constraint, channel, decoder]
)
...
new_encoder = MLPEncoder(
    in_features=input_dim,
    out_features=hidden_dim,
    hidden_dims=[32, 24]  # Deeper network
)

# Create a new sequential model with the updated encoder
enhanced_model = SequentialModel(
    steps=[new_encoder, constraint, channel, decoder]
)
...
for i, module in enumerate(sequential_model.steps):
    ...
    output_stats.append({
        ...
        'min': current_output.min().item(),
        'max': current_output.max().item()
    })
...
for stats in output_stats:
    print(f"{stats['module']}: mean={stats['mean']:.3f}, std={stats['std']:.3f}, " 
...
residual_model = ResidualSequentialModel(
    steps=[encoder, constraint, channel, decoder]
)
...
def train_sequential_model(model, optimizer, num_epochs=50, batch_size=32,
                         input_dim=10, snr_range=(0, 20)):
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training Loss for Sequential Model')
plt.grid(True)
"""
