...
forward_encoder = MLPEncoder(in_features=message_dim, out_features=code_dim, hidden_dims=[20, 15])  # Change to only take message as input, matching what's provided
...
feedback_model = FeedbackChannelModel(
    encoder=forward_encoder, forward_channel=forward_channel, decoder=decoder, feedback_generator=feedback_encoder, feedback_channel=feedback_channel, feedback_processor=feedback_encoder, max_iterations=3  # Using feedback_encoder as the processor for simplicity  # Adding explicit iteration count
)
...
standard_encoder = MLPEncoder(in_features=message_dim, out_features=code_dim, hidden_dims=[20, 15])
...
def direct_transmission(messages, encoder, constraint, channel, decoder, snr):
    """Simulates direct transmission without feedback.

    Args:
        messages (torch.Tensor): The messages to transmit.
    ...
    Returns:
        torch.Tensor: The reconstructed messages.
    """
...
for forward_snr in forward_snr_values:
    # With feedback
    with torch.no_grad():
        feedback_reconstructed = feedback_model(messages, forward_snr=forward_snr, feedback_snr=feedback_snr)
        feedback_error = mse_loss(feedback_reconstructed, messages).item()
        feedback_errors.append(feedback_error)

    # Without feedback
    with torch.no_grad():
        direct_reconstructed = direct_transmission(messages, standard_encoder, standard_constraint, standard_channel, standard_decoder, forward_snr)
...
def get_feedback_signal(model, messages, forward_snr, feedback_snr):
    """Extracts the feedback signal generated by the model.

    Args:
        model (FeedbackChannelModel): The feedback channel model.
        messages (torch.Tensor): The input messages.
        forward_snr (float): The forward channel SNR in dB.
        feedback_snr (float): The feedback channel SNR in dB.

    Returns:
        torch.Tensor: The generated feedback signal.
    """
    # First encode and send the message
    # Note: Accessing internal components like forward_encoder might be fragile.
    # A better approach might be to modify the model's forward pass to return intermediates.
    # Assuming the model structure allows this access for demonstration.
    initial_feedback = torch.zeros(messages.shape[0], model.feedback_dim, device=messages.device)
    encoder_input = torch.cat([messages, initial_feedback], dim=1)
    code = model.encoder(encoder_input)
    constrained_code = model.forward_constraint(code)
    received = model.forward_channel(constrained_code, snr=forward_snr)

    # Generate feedback
    feedback = model.feedback_generator(received)

    return feedback


# Get feedback for some example messages
example_feedback = get_feedback_signal(feedback_model, messages[:5], forward_snr=10, feedback_snr=10)
...
def train_feedback_model(model, optimizer, num_epochs=50, batch_size=32, message_dim=10, forward_snr_range=(0, 20), feedback_snr_range=(5, 15)):
