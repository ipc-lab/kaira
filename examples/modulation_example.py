...
def basic_modulation_example():
    ...
    noise = torch.complex(
        torch.randn_like(symbols.real) * noise_std, torch.randn_like(symbols.imag) * noise_std
    )
...
def ber_analysis_example():
    ...
    results, fig = compare_modulation_schemes(
        schemes, snr_range, n_bits=10000, title="BER Comparison of Modulation Schemes"
    )
...
def batch_processing_example():
    ...
    # Add noise
    noise = torch.complex(
        torch.randn_like(symbols.real) * 0.05, torch.randn_like(symbols.imag) * 0.05
    )
    noisy_symbols = symbols + noise
