...
def compute_losses(d_output):
    ...
    losses = {
        'Vanilla GAN': vanilla_gan.forward_generator(d_output),
        'WGAN': wgan.forward_generator(d_output),
        'LSGAN': lsgan.forward_generator(d_output),
        'Hinge': hinge.forward_generator(d_output)
    }
...
def compute_d_losses(real_out, fake_out):
    ...
    losses = {
        'Vanilla GAN': vanilla_gan.forward_discriminator(real_batch, fake_batch),
        'WGAN': wgan.forward_discriminator(real_batch, fake_batch),
        'LSGAN': lsgan.forward_discriminator(real_batch, fake_batch),
        'Hinge': hinge.forward_discriminator(real_batch, fake_batch)
    }
    return {k: v.item() for k, v in losses.items()}
...
for idx, (name, loss_surface) in enumerate(Z.items(), 1):
    ...
    surf = ax.plot_surface(X.numpy(), Y.numpy(), loss_surface.numpy(), 
                          cmap='viridis')
