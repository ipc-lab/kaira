...
signal = (0.5 * np.sin(2 * np.pi * 3 * t) + 
          0.3 * np.sin(2 * np.pi * 7 * t) +
          0.2 * np.cos(2 * np.pi * 11 * t))
...
for i, (snr_db, awgn_output, laplacian_output) in enumerate(zip(
        [level for level, _, _ in channels], 
        [output for _, output in awgn_outputs],
        [output for _, output in laplacian_outputs])):
...
plt.subplot(1, 2, 2)
plt.semilogy(np.sort(np.abs(awgn_noise)), np.linspace(1, 0, len(awgn_noise)), 'b-', 
           linewidth=2, label='Gaussian Noise')
plt.semilogy(np.sort(np.abs(laplacian_noise)), np.linspace(1, 0, len(laplacian_noise)), 'r-', 
           linewidth=2, label='Laplacian Noise')
plt.grid(True)
