...
import torch
from kaira.metrics import BaseMetric, BER, PSNR
...
plt.axhline(y=noisy_signal.min().item() + (noisy_signal.max() - noisy_signal.min()).item()/2, 
           color='g', linestyle='--', label='Adaptive Threshold')
...
plt.axhline(y=0, color='r', linestyle='-', label='Standard Threshold')
plt.axhline(y=offset_signal.min().item() + (offset_signal.max() - offset_signal.min()).item()/2, 
           color='g', linestyle='--', label='Adaptive Threshold')
plt.grid(True, alpha=0.3)
plt.xlabel('Sample')
plt.ylabel('Signal')
...
plt.axhline(y=0, color='r', linestyle='-', label='Standard Threshold')
plt.axhline(y=scaled_signal.min().item() + (scaled_signal.max() - scaled_signal.min()).item()/2, 
           color='g', linestyle='--', label='Adaptive Threshold')
plt.grid(True, alpha=0.3)
plt.xlabel('Sample')
plt.ylabel('Signal')
...
class QualityOfServiceMetric(BaseMetric):
    ...
    def forward(self, error_rate, latency_ms, throughput_mbps):
        ...
        qos_score = (self.error_weight * norm_error +
...
class MeanOpinionScore(BaseMetric):
    ...
    Implements a simplified version of the E-model (ITU-T G.107) to estimate
    user satisfaction with voice quality based on technical parameters.
    """
    ...
    def _calculate_r_factor(self, latency_ms, packet_loss_percent, jitter_ms):
        ...
        id_factor = torch.where(delay_mask, 
                               0.02 * (latency_ms - 150.0), 
                               id_factor)
        
        # Additional penalty for delays > 300ms
        severe_delay_mask = latency_ms > 300.0
        id_factor = torch.where(severe_delay_mask,
                               id_factor + 0.1 * (latency_ms - 300.0),
                               id_factor)
...
contour_lines = plt.contour(lat_grid.numpy(), loss_grid.numpy(), mos_values.numpy(), 
                          levels=[1.0, 2.0, 3.0, 3.6, 4.0, 4.3], 
