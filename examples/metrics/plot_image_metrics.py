...
if not SAMPLE_IMAGES_DIR.exists() or not list(SAMPLE_IMAGES_DIR.glob("*.*")):
    raise RuntimeError(
        "Test images not found. Please run:\n" +
...
def evaluate_all_metrics(original, distorted):
    ...
    return {
        'PSNR': psnr(distorted, original),  # Now returns scalar mean
        'SSIM': ssim(distorted, original),  # Now returns scalar mean
        'MS-SSIM': ms_ssim(distorted, original),  # Now returns scalar mean
        'LPIPS': lpips(distorted, original)  # Now returns scalar mean
    }
