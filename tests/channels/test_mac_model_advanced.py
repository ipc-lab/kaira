...
class ComplexDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.net is None or self.net[0].in_features != input_features:
            self.net = nn.Sequential(nn.Linear(input_features, self.hidden_dim), nn.ReLU(), nn.Linear(self.hidden_dim, self.output_dim))
...
def mac_components():
    ...
    return {"channel": channel, "power_constraint": power_constraint, "encoder": ComplexEncoder, "decoder": ComplexDecoder, "num_devices": 3}
...
def test_mac_model_device_dropouts():
    ...
    model = MultipleAccessChannelModel(channel=channel, power_constraint=power_constraint, encoder=ComplexEncoder, decoder=ComplexDecoder, num_devices=5)
