...
def test_transcoder_model_initialization():
    ...
    # Initialize model
    model = Kurmukova2025TransCoder(
        encoder_tc=encoder_tc,
        encoder_ec=encoder_ec,
        constraint=constraint,
        modulator=modulator,
        channel=channel,
        demodulator=demodulator,
        decoder_tc=decoder_tc,
        decoder_ec=decoder_ec,
        n_iterations=2
    )
...
@pytest.mark.parametrize("use_tc_encoder, use_tc_decoder", [
    (True, True),
    (True, False),
    (False, True),
    (False, False)
