...
def model_with_refinement():
    ...
    model = DeepJSCCFeedbackModel(
        ...
        channel_type="awgn",
        feedback_snr=15,
        refinement_layer=True,  # Enable refinement layer
        layer_id=1,  # Non-zero layer ID for refinement
        forward_channel=forward_channel,
        feedback_channel=feedback_channel
    )
...
def model_no_feedback_noise():
    ...
    return DeepJSCCFeedbackModel(
        ...
        conv_depth=64,
        channel_type="awgn",
        feedback_snr=None,  # No feedback noise
        refinement_layer=False,
        layer_id=0,
        forward_channel=forward_channel,
...
def test_default_forward_channel_initialization():
    ...
    test_snr = 15.0
    model = DeepJSCCFeedbackModel(
        channel_snr=test_snr,
        conv_depth=64,
        channel_type="awgn",
        feedback_snr=10,
        refinement_layer=False,
        layer_id=0,
        forward_channel=None,  # Explicitly set to None to test initialization
        feedback_channel=IdentityChannel()
    )
...
def test_default_feedback_channel_initialization_perfect():
    """Test that the feedback channel is properly initialized as IdentityChannel when feedback_snr is None."""
    model = DeepJSCCFeedbackModel(
        channel_snr=10,
        conv_depth=64,
        channel_type="awgn",
        feedback_snr=None,  # None should result in IdentityChannel
        refinement_layer=False,
        layer_id=0,
        forward_channel=AWGNChannel(snr_db=10),
        feedback_channel=None  # Explicitly set to None to test initialization
    )
...
def test_default_feedback_channel_initialization_noisy():
    """Test that the feedback channel is properly initialized as AWGNChannel when feedback_snr is provided."""
    test_fb_snr = 20.0
    model = DeepJSCCFeedbackModel(
        channel_snr=10,
        conv_depth=64,
        channel_type="awgn",
        feedback_snr=test_fb_snr,  # Should result in AWGNChannel with this SNR
        refinement_layer=False,
        layer_id=0,
        forward_channel=AWGNChannel(snr_db=10),
        feedback_channel=None  # Explicitly set to None to test initialization
