...
def test_compute_bits_compressai():
    ...
    mock_result = {"likelihoods": {"y": torch.tensor([[[0.5, 0.25], [0.5, 0.5]], [[0.5, 0.5], [0.5, 0.5]]]), "z": torch.tensor([[[0.25, 0.5]], [[0.5, 0.5]]])}, "x_hat": torch.ones(2, 3, 8, 8)}  # Batch size 2  # Batch size 2  # Batch size 2
...
def test_max_bits_warning(mock_factory, sample_image, mock_compressai_model):
    ...
    with patch.object(compressor, "compute_bits_compressai", return_value=torch.tensor([bits_per_image])):
        with pytest.warns(UserWarning, match="Some images exceed the max_bits_per_image constraint"):
            compressor(sample_image)
...
def test_compressed_data_with_bit_constraint(mock_factory, sample_image, mock_compressai_model):
    ...
    try:
        ...
        compressor = NeuralCompressor(method="bmshj2018_factorized", max_bits_per_image=1000, return_compressed_data=True)  # Any value is fine since we're mocking
...
def test_bit_constrained_stats_collection(mock_factory, sample_image, mock_compressai_model):
    ...
    compressor = NeuralCompressor(method="bmshj2018_factorized", max_bits_per_image=max_bits, collect_stats=True)
    ...
    try:
        ...
        def patched_forward(self, x, *args, **kwargs):
            ...
            img_stats = [
                {"quality": 8, "bits": bits_per_image.item(), "bpp": bits_per_image.item() / (x.shape[2] * x.shape[3]), "compression_ratio": (x.shape[1] * x.shape[2] * x.shape[3] * 8) / bits_per_image.item()},  # Highest quality that fits within the constraint
                {"quality": 7, "bits": (bits_per_image * 0.8).item(), "bpp": (bits_per_image * 0.8).item() / (x.shape[2] * x.shape[3]), "compression_ratio": (x.shape[1] * x.shape[2] * x.shape[3] * 8) / (bits_per_image * 0.8).item()},  # Lower quality for second image
            ]
...
def test_lowest_quality_fallback(mock_factory, sample_image, mock_compressai_model):
    ...
    try:
        ...
        def patched_forward(self, x, *args, **kwargs):
            ...
            img_stats = [
                {"quality": 1, "bits": bits_per_image.item(), "bpp": bits_per_image.item() / (x.shape[2] * x.shape[3]), "compression_ratio": (x.shape[1] * x.shape[2] * x.shape[3] * 8) / bits_per_image.item()},  # Lowest quality
                {"quality": 1, "bits": bits_per_image.item(), "bpp": bits_per_image.item() / (x.shape[2] * x.shape[3]), "compression_ratio": (x.shape[1] * x.shape[2] * x.shape[3] * 8) / bits_per_image.item()},  # Lowest quality
            ]
...
def test_bit_constraint_quality_selection(mock_factory, sample_image, mock_compressai_model):
    ...
    max_bits = 500
    compressor = NeuralCompressor(method="bmshj2018_factorized", max_bits_per_image=max_bits, return_bits=True)
...
def test_compressed_data_storage_in_bit_constraint_mode(mock_factory, sample_image, mock_compressai_model):
    ...
    max_bits = 400
    compressor = NeuralCompressor(method="bmshj2018_factorized", max_bits_per_image=max_bits, return_compressed_data=True)

    # Expected compressed data result
    expected_compressed_data = [{"strings": {"y": b"quality1_data"}, "shape": {"y": [8, 8]}}, {"strings": {"y": b"quality3_data"}, "shape": {"y": [8, 8]}}, {"strings": {"y": b"quality2_data"}, "shape": {"y": [8, 8]}}]
...
def test_stats_collection_in_bit_constraint_mode(mock_factory, sample_image, mock_compressai_model):
    ...
    expected_stats = {"total_bits": sum(expected_bits), "avg_quality": sum(expected_qualities) / len(expected_qualities), "model_name": "bmshj2018_factorized", "metric": "mse", "processing_time": 0.1, "img_stats": [], "avg_bpp": 0.0, "avg_compression_ratio": 0.0}  # Arbitrary value
    ...
    for i in range(batch_size):
        expected_stats["img_stats"].append({"quality": expected_qualities[i], "bits": expected_bits[i], "bpp": expected_bits[i] / img_area, "compression_ratio": original_size / expected_bits[i]})
...
def test_return_values_with_all_flags(mock_factory, sample_image, mock_compressai_model):
    ...
    test_configs = [{"return_bits": True, "return_compressed_data": True}, {"return_bits": True, "return_compressed_data": False}, {"return_bits": False, "return_compressed_data": True}, {"return_bits": False, "return_compressed_data": False}]

    for config in test_configs:
        # Create a compressor with the current flag configuration
        compressor = NeuralCompressor(method="bmshj2018_factorized", max_bits_per_image=400, return_bits=config["return_bits"], return_compressed_data=config["return_compressed_data"])
        ...
        try:
            ...
            def patched_forward(self, x, *args, **kwargs):
                ...
                if self.return_bits and self.return_compressed_data:
                    return (torch.ones_like(x), torch.tensor([350.0, 380.0]), [{"data": "compressed1"}, {"data": "compressed2"}])
                elif self.return_bits:
