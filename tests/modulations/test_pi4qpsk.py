...
def test_single_input_distance_calculation():
    """Test specifically the lines handling single input distance calculation in
    Pi4QPSKDemodulator."""
    # Create a controlled environment for testing just the distance calculation
    mod = Pi4QPSKModulator()
...
def test_non_batched_hard_decision_bit_assignment():
    """Test specifically the mask.item() condition for bit pattern assignment in non-batched hard
    decision case."""
    ...
    noise_level = 0.05
    noisy_signal = test_signal + noise_level * (torch.randn_like(test_signal.real) + 1j * torch.randn_like(test_signal.imag))
...
def test_pi4qpsk_demodulator_single_input_lines():
    """Test specifically the single input distance calculation and mask.item() bit assignment in
    Pi4QPSKDemodulator."""
...
def test_pi4qpsk_single_input_distance_calculation():
    ...
    arbitrary_signal = torch.complex(torch.tensor([0.5, -0.3, 0.7, -0.9]), torch.tensor([0.2, 0.8, -0.4, -0.6]))
