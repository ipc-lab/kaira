...
from kaira.modulations import IdentityDemodulator, IdentityModulator
...
class TestIdentityModulationComprehensive:
    ...
    def test_identity_modulator_forward(self):
        ...
        inputs = [torch.tensor([0, 1, 0, 1]), torch.tensor([[0, 1], [1, 0]]), torch.tensor([[[0, 1], [1, 0]], [[1, 1], [0, 0]]])]
    ...
    def test_identity_modulator_demodulator_roundtrip(self):
        ...
        inputs = [torch.tensor([0.0, 1.0, 0.0, 1.0]), torch.tensor([0.0, 0.0, 1.0, 1.0]), torch.tensor([0.0, 1.0, 1.0, 0.0]), torch.randn(10)]  # Even non-binary values should pass through
