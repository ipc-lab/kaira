...
def test_qam_modulator_perfect_square_validation():
    ...
    for order in valid_orders:
        QAMModulator(order=order)  # Should not raise
...
def test_4qam_constellation_structure():
    ...
    expected_points = torch.tensor([-1 - 1j, -1 + 1j, 1 - 1j, 1 + 1j])
...
def test_qam_demodulation_with_noise(order, device):
    ...
    noisy_symbols = symbols + torch.complex(torch.randn_like(symbols.real) * noise_std, torch.randn_like(symbols.imag) * noise_std)
