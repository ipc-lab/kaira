...
class TestQAMModulationEdgeCases:
    """Test edge cases for QAM modulation."""

    def test_qam_modulator_invalid_order(self):
        """Test QAM modulator with invalid order values."""
        # Order must be a perfect square for QAM
        with pytest.raises(ValueError, match="must be a perfect square"):
            QAMModulator(order=10)  # Not a perfect square
        
        # Order must be a power of 4 for rectangular QAM
        with pytest.raises(ValueError, match="must be a perfect square"):
            QAMModulator(order=32)  # Not a power of 4

    def test_qam_modulator_invalid_inputs(self):
        """Test QAM modulator with invalid inputs."""
        ...
        with pytest.raises(ValueError, match="must be divisible"):
            ...
            modulator(torch.tensor([0, 1, 0, 1, 0]))  # 5 bits, not divisible by log2(16)=4

    def test_qam_demodulator_edge_cases(self):
        """Test QAM demodulator with edge cases."""
        ...
        # Should still demodulate based on decision regions
        result = demodulator(small_input)
        assert result.shape[1] == 4  # Each symbol maps to 4 bits for 16-QAM
    ...
    def test_qam_modulator_custom_constellation(self):
        """Test QAM modulator with custom constellation points."""
        # Create a custom 4-QAM constellation
        custom_const = torch.tensor([1+1j, 1-1j, -1+1j, -1-1j], dtype=torch.complex64)
        
        modulator = QAMModulator(constellation=custom_const)
    ...
    def test_qam_modulator_demodulator_roundtrip(self):
        """Test round trip conversion with different QAM orders."""
        ...
        for order in [4, 16, 64, 256]:
            ...
            modulator = QAMModulator(order=order)
            demodulator = QAMDemodulator(order=order)
    ...
    def test_qam_constellation_normalization(self):
        """Test that QAM constellation is properly normalized."""
        # For different orders, verify the average power is 1
        for order in [4, 16, 64, 256]:
            modulator = QAMModulator(order=order)
            constellation = modulator.constellation
            
            # Calculate average power
            avg_power = torch.mean(torch.abs(constellation)**2).item()
            
            # Average power should be approximately 1
            assert abs(avg_power - 1.0) < 1e-6

    def test_qam_gray_coding(self):
        """Test that QAM modulator uses Gray coding."""
        # For 16-QAM, verify that adjacent symbols differ by only one bit
        modulator = QAMModulator(order=16)
        
        # Get the constellation and generate bit mappings for each point
        constellation = modulator.constellation
        
        # Create all 4-bit patterns
        all_bits = torch.tensor([
            [int(b) for b in format(i, '04b')] for i in range(16)
        ], dtype=torch.float)
        
        # Modulate all patterns
        symbols = modulator(all_bits.reshape(-1)).reshape(16)
        
        # For each symbol, find its nearest neighbors in the constellation
        # and verify that they differ by only one bit
        for i in range(16):
            bits_i = all_bits[i]
            symbol_i = symbols[i]
            
            # Find nearest neighbors (using Euclidean distance in complex plane)
            distances = torch.abs(symbols - symbol_i)
            
            # Sort by distance
            sorted_indices = torch.argsort(distances)
            
            # Skip the first one (distance to itself is 0)
            for j in sorted_indices[1:5]:  # Check a few closest neighbors
                bits_j = all_bits[j]
                
                # Calculate Hamming distance between bit patterns
                hamming_distance = torch.sum(bits_i != bits_j).item()
                
                # If they are adjacent in the constellation (small Euclidean distance),
                # they should have a Hamming distance of 1 (Gray coding property)
                if distances[j] < 0.5:  # Adjacent points in QAM constellation
                    assert hamming_distance == 1, f"Gray coding violated between {bits_i} and {bits_j}"

    def test_qam_different_sized_dimensions(self):
        """Test QAM with different input dimensions."""
        modulator = QAMModulator(order=16)
        demodulator = QAMDemodulator(order=16)
        
        # 1D input (flat array of bits)
        bits_1d = torch.tensor([0, 1, 1, 0, 0, 0, 1, 1])
        symbols_1d = modulator(bits_1d)
        assert symbols_1d.shape[0] == bits_1d.shape[0] // 4
        
        # 2D input (multiple rows of bits)
        bits_2d = torch.tensor([
            [0, 1, 1, 0],
            [0, 0, 1, 1],
        ])
        symbols_2d = modulator(bits_2d)
        assert symbols_2d.shape[0] == bits_2d.shape[0]
        
        # Round trip conversion for both cases
        recovered_1d = demodulator(symbols_1d)
        recovered_2d = demodulator(symbols_2d)
        
        assert torch.all(bits_1d.reshape(-1, 4) == recovered_1d)
        assert torch.all(bits_2d == recovered_2d)