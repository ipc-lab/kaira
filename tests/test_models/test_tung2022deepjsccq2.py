# tests/test_models/test_deepjsccq2.py
import pytest
import torch

from kaira.models.image.tung2022_deepjscc_q2 import DeepJSCCQ2Encoder, Tung2022DeepJSCCQ2Decoder


@pytest.fixture
def encoder2():
    """Fixture for creating a DeepJSCCQ2Encoder."""
    return DeepJSCCQ2Encoder(N=32, M=64, csi_length=1)
...
def test_deepjsccq2_encoder_initialization(encoder2):
    ...
    assert isinstance(encoder2, DeepJSCCQ2Encoder)
...
def test_deepjsccq2_encoder_forward(encoder2, sample_image, sample_snr):
    ...
    encoded_image = encoder2((sample_image, sample_snr))
...
def test_deepjsccq2_decoder_forward(decoder2, encoder2, sample_image, sample_snr):
    ...
    encoded_image = encoder2((sample_image, sample_snr))
    decoded_image = decoder2((encoded_image, sample_snr))
...
def test_deepjsccq2_encoder_decoder_different_sizes(N, M, csi_length):
    ...
    decoded_image = decoder((encoded_image, sample_snr))
    assert decoded_image.shape == sample_image.shape
