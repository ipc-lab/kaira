...
class TestMetricsUtils:
    ...
    def test_compute_multiple_metrics(self):
        ...
        # Create test metrics
        metrics = {"metric1": MockMetric(0.8), "metric2": MockMetric(0.6), "metric3": MockMetric(0.9, return_with_stats=True)}
    ...
    def test_format_metric_results(self):
        ...
        results = {"metric1": torch.tensor(0.7532), "metric2": 0.8456, "metric3": (torch.tensor(0.6), torch.tensor(0.1))}  # With std
        ...
        results = {"metric1": (torch.tensor(0.7532), torch.tensor(0.0123)), "metric2": (0.8456, 0.0234), "metric3": 0.7}  # Plain number
        formatted = format_metric_results(results)
    ...
    def test_visualize_metrics_comparison(self, monkeypatch):
        ...
        results_list = [{"metric1": torch.tensor(0.75), "metric2": torch.tensor(0.85)}, {"metric1": torch.tensor(0.78), "metric2": torch.tensor(0.82)}]
        visualize_metrics_comparison(results_list, labels)
    ...
    def test_visualize_metrics_comparison_with_tensor_tuples(self, monkeypatch):
        ...
        results_list = [{"metric1": (torch.tensor(0.8), torch.tensor(0.05)), "metric2": (torch.tensor(0.6), torch.tensor(0.03))}, {"metric1": (torch.tensor(0.7), torch.tensor(0.04)), "metric2": (torch.tensor(0.5), torch.tensor(0.02))}]
    ...
    def test_print_metric_table(self, capsys):
        ...
        table = [["Metric", "Mean", "Std"], ["metric1", "0.7532", "0.0123"], ["metric2", "0.8456", "0.0234"]]
    ...
    def test_summarize_metrics_over_batches(self):
        ...
        metrics_history = [{"metric1": 0.8, "metric2": (0.6, 0.1)}, {"metric1": 0.7, "metric2": (0.5, 0.1)}, {"metric1": 0.9, "metric2": (0.7, 0.1)}]
    ...
    def test_compute_multiple_metrics_direct_call(self):
        ...
        metrics = {"direct_metric": DirectMockMetric(0.7)}
    ...
    def test_visualize_metrics_comparison_single_value(self, monkeypatch):
        ...
        results_list = [{"single_value_metric": torch.tensor(0.85)}]
    ...
    def test_print_metric_table_with_empty_rows(self, capsys):
        ...
        table = [["Metric", "Mean", "Std"], ["metric1", "0.7532", "0.0123"], [], ["metric2", "0.8456", "0.0234"], []]  # Empty row  # Another empty row
    ...
    def test_print_metric_table_with_missing_columns(self, capsys):
        ...
        table = [["Metric", "Mean", "Std"], ["metric1", "0.7532", "0.0123"], ["metric2", "0.8456"], ["metric3"]]  # Missing Std column  # Missing Mean and Std columns
    ...
    def test_print_metric_table_empty_cases(self, capsys):
        ...
        print_metric_table([["Metric", "Value"], [], ["metric1", "0.123"], [], ["metric2", "0.456"]])
        captured = capsys.readouterr()
        assert "Metric" in captured.out
        assert "Value" in captured.out
