...
def test_vgg_loss(sample_images, sample_target_images, monkeypatch, with_input):
    ...
    class MockModule(nn.Module):
        def __init__(self):
            super().__init__()
            self._modules = {"3": nn.Identity(), "8": nn.Identity(), "15": nn.Identity(), "22": nn.Identity()}
    ...
    mock_weights = object()
    monkeypatch.setattr("torchvision.models.VGG16_Weights", type("obj", (object,), {"DEFAULT": mock_weights}))
...
def test_vgg_loss_layer_weights(monkeypatch):
    ...
    assert loss_fn_default.layer_weights == {"conv1_2": 0.1, "conv2_2": 0.2, "conv3_3": 0.4, "conv4_3": 0.3}

    # Test custom layer weights
    custom_weights = {"conv1_2": 0.15, "conv2_2": 0.25, "conv3_3": 0.35, "conv4_3": 0.25}
