...
This module defines the IdentityConstraint which is a passthrough constraint that does not
modify the input signal. It's useful as a no-op constraint or as a baseline for comparison.
"""
...
class IdentityConstraint(BaseConstraint):
    ...
    This is a simple passthrough constraint that does not modify the input signal.
    It can be used when a constraint is expected in an interface but no actual 
