...
class STFTLoss(nn.Module):
    ...
    This module calculates the STFT loss between the input and target audio signals,
    combining spectral convergence loss and log STFT magnitude loss.
    """
...
class MultiResolutionSTFTLoss(nn.Module):
    ...
    def __init__(self, fft_sizes=[512, 1024, 2048], hop_sizes=[128, 256, 512], 
                 win_lengths=[512, 1024, 2048], window='hann'):
...
class MelSpectrogramLoss(nn.Module):
    ...
    def __init__(self, sample_rate=22050, n_fft=1024, hop_length=256, n_mels=80,
                 f_min=0.0, f_max=8000.0, log_mel=True):
...
class FeatureMatchingLoss(nn.Module):
    ...
    def forward(self, x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for i, layer_idx in enumerate(self.layers):
            handles.append(list(self.model.children())[layer_idx].register_forward_hook(
        ...
        for i in range(len(self.layers)):
            ...
            loss += self.weights[i] * F.l1_loss(
                activations_x[layer_name], 
                activations_target[layer_name]
            )
...
class AudioContrastiveLoss(nn.Module):
    ...
    This module calculates a contrastive loss to bring similar audio samples closer in feature space.
    """
