...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
                 ...
                 shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, act_layer=nn.GELU,
...
class Yang2024SwinJSCCEncoder(BaseModel):
    ...
    This encoder uses a Swin Transformer architecture with adaptive modulation 
    capabilities for signal-to-noise ratio (SNR) and rate adaptation.
    """
    def __init__(
        self, 
        ...
        self.layers = self._build_layers(
            embed_dims, depths, num_heads, window_size,
            qkv_bias, qk_scale, norm_layer
        )
    ...
    def _build_layers(self, embed_dims, depths, num_heads, window_size,
                    qkv_bias, qk_scale, norm_layer):
    ...
    def forward(
        ...
        model_mode: str = 'SwinJSCC_w/o_SAandRA',
        ...
        with torch.cuda.amp.autocast() if self.use_mixed_precision and torch.cuda.is_available() else nullcontext():
            ...
            if model_mode == 'SwinJSCC_w/o_SAandRA':
                ...
                raise ValueError(f"Unknown model mode: {model_mode}. Valid modes are: "
                               "'SwinJSCC_w/o_SAandRA', 'SwinJSCC_w/_SA', 'SwinJSCC_w/_RA', 'SwinJSCC_w/_SAandRA'")
    ...
    def get_model_size(self) -> Dict[str, Union[int, float]]:
        ...
        return {
            ...
            'param_size_mb': num_params * 4 / 1024 / 1024,  # Assuming float32
...
class Yang2024SwinJSCCDecoder(BaseModel):
    ...
    This decoder takes the encoded representation from Yang2024SwinJSCCEncoder and 
    reconstructs the original image. It supports SNR adaptation for
    robust performance across varying channel conditions.
    """
    def __init__(
        self,
        img_size: Union[int, Tuple[int, int]],
    ...
    def _forward_with_snr(self, x: torch.Tensor, snr: float, 
                          intermediate_features: Optional[Dict] = None) -> torch.Tensor:
...
class nullcontext:
    """A context manager that does nothing. Used as a placeholder."""
    def __enter__(self):
        pass
...
def build_model(config):
    ...
    Validates the model by running a test input and reports stats.
