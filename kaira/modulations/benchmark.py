...
def compare_modulation_schemes(
    ...
    for i, (name, (mod, demod)) in enumerate(modulators.items()):
        ...
        plot_ber_curve(
            ber_results, name, theoretical=theoretical, ax=ax, color=color, marker=marker
        )
...
def benchmark_modulation_schemes(
    ...
    for name, (mod, demod) in modulators.items():
        ...
        throughput = measure_throughput(
            modulator=mod, demodulator=demod, n_bits=n_bits, batch_size=batch_size, device=device
        )
        ...
        results.append(
            {
                ...
                "Demodulation Throughput (Mbps)": throughput.get(
                    "demodulation_throughput_mbps", float("nan")
                ),
            }
        )
