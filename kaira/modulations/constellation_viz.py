...
class ConstellationVisualizer:
    ...
    def plot_basic(self, show_grid: bool = True, show_labels: bool = True) -> plt.Figure:
        ...
        max_val = max(np.max(np.abs(constellation_np.real)), np.max(np.abs(constellation_np.imag)))
        limit = max_val * 1.2
    ...
    def plot_decision_regions(self, resolution: int = 200) -> plt.Figure:
        ...
        im = self.ax.pcolormesh(X, Y, closest_indices, cmap=cmap, alpha=0.3, shading="auto")
    ...
    def plot_with_noise(
        self, snr_db: float = 15.0, n_points: int = 1000, show_density: bool = True
    ) -> plt.Figure:
    ...
    def plot_ber_estimation(self, snr_db_range: List[float] = None) -> plt.Figure:
        ...
        for snr_db in snr_db_range:
            ...
            decisions = np.argmin(
                np.abs(received[:, np.newaxis] - self.constellation.numpy()), axis=1
            )
    ...
    def animate_phase_rotation(
        self, n_frames: int = 100, rotation_cycles: float = 1.0, interval: int = 50
    ) -> animation.Animation:
    ...
    def plot_bit_reliability(self, snr_db: float = 10.0, n_points: int = 10000) -> plt.Figure:
        ...
        fig.suptitle(f"{self.title} Bit Reliability Analysis (SNR={snr_db}dB)", fontsize=16)
        plt.tight_layout()
    ...
    def plot_eye_diagram(
        ...
        if pulse_type.lower() == "rect":
            ...
            t = np.arange(
                -filter_span / 2, filter_span / 2 + 1 / samples_per_symbol, 1 / samples_per_symbol
            )
        ...
        fig.suptitle(f"{self.title} Eye Diagram (SNR={snr_db}dB, β={beta})", fontsize=14)
        plt.tight_layout()
    ...
    def plot_spectral_efficiency(self, modulation_names: List[str] = None) -> plt.Figure:
        ...
        for name in modulation_names:
            if (
                name.lower().endswith("psk")
                or name.lower().endswith("qam")
                or name.lower().endswith("point")
            ):
