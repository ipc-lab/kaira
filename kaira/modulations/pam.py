...
class PAMModulator(BaseModulator):
    ...
    Standard PAM modulation with uniform amplitude levels. Can use Gray coding 
    for bit-to-symbol mapping, and supports normalization to unit average energy.
    """
...
class PAMDemodulator(BaseDemodulator):
    ...
    def _min_distance_to_levels(self, y_real: torch.Tensor, levels: torch.Tensor, 
                               noise_var: Union[float, torch.Tensor]) -> torch.Tensor:
    ...
    def _compute_llrs(self, y_real: torch.Tensor, noise_var: Union[float, torch.Tensor]) -> torch.Tensor:
        ...
        # Prepare LLR output
        llrs = torch.zeros((*batch_shape, n_symbols * self._bits_per_symbol), 
