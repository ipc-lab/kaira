...
def plot_constellation(
    ...
    if labels is not None:
        ...
        for label in unique_labels:
            mask = labels == label
            ax.scatter(symbols.real[mask], symbols.imag[mask], alpha=alpha, label=f"{label}")
        ax.legend()
    else:
    ...
    if len(symbols) <= 64:
        for i, symbol in enumerate(symbols):
            ...
            ax.annotate(label, (symbol.real, symbol.imag), fontsize=8, ha="center", va="center")
...
def calculate_theoretical_ber(
    snr_db: Union[float, List[float], np.ndarray], modulation: str
) -> np.ndarray:
