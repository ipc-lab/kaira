...
class Pi4QPSKModulator(BaseModulator):
    ...
    on alternating symbols, providing better spectral properties.
    """
    ...
    def plot_constellation(self, both: bool = True, **kwargs) -> Union[plt.Figure, Tuple[plt.Figure, plt.Figure]]:
        ...
        if both:
            ...
            return plot_constellation(
                ...
                title="π/4-QPSK Combined Constellation",
                **kwargs
            )
...
class Pi4QPSKDemodulator(BaseDemodulator):
    ...
    def __init__(self, normalize: bool = True) -> None:
        ...
        self.register_buffer('combined_constellation', 
                             torch.cat([self.modulator.constellation_std, 
                                       self.modulator.constellation_rot]))
    ...
    def forward(self, y: torch.Tensor, noise_var: Optional[Union[float, torch.Tensor]] = None) -> torch.Tensor:
        ...
        if noise_var is None:
            ...
            expanded_const = self.combined_constellation.expand(*([1] * len(batch_shape)), 
                                                              symbol_shape, 8)  # (..., N, 8)
