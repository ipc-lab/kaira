...
def plot_channel_response(
    ...
    **plot_kwargs,
    ...
    defaults = {"marker": "o", "markersize": 3, "alpha": 0.7, "linewidth": 1.5}
...
def plot_constellation(
    original: torch.Tensor, received: torch.Tensor, ax: Optional[plt.Axes] = None
) -> plt.Axes:
    ...
    ax.scatter(orig_np.real, orig_np.imag, c="blue", marker="o", s=50, alpha=0.7, label="Original")

    # Plot received constellation points
    ax.scatter(recv_np.real, recv_np.imag, c="red", marker="x", s=30, alpha=0.5, label="Received")

    # Optional: draw lines between original and received
    for i in range(min(len(orig_np), 100)):  # Limit to 100 lines to avoid clutter
        ax.plot(
            [orig_np[i].real, recv_np[i].real], [orig_np[i].imag, recv_np[i].imag], "k-", alpha=0.2
        )
...
def plot_impulse_response(
    channel: BaseChannel, length: int = 20, ax: Optional[plt.Axes] = None
) -> plt.Axes:
